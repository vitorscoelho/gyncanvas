...
object Earcut {
    ...
    private fun earcutLinked(
        ...
        if (pass == Int.MIN_VALUE && invSize != Double.MIN_VALUE) indexCurve(ear, minX, minY, invSize)
        var stop = ear
        ...
        while (ear!!.prev !== ear!!.next) {
            val prev = ear!!.prev
            val next = ear.next
            if (if (invSize != Double.MIN_VALUE) isEarHashed(ear, minX, minY, invSize) else isEar(ear)) {
    ...
    private fun intersects(p1: Node?, q1: Node?, p2: Node?, q2: Node?): Boolean {
        ...
        if (o1 == 0.0 && onSegment(p1, p2, q1)) return true // p1, q1 and p2 are collinear and p2 lies on p1q1
        if (o2 == 0.0 && onSegment(p1, q2, q1)) return true // p1, q1 and q2 are collinear and q2 lies on p1q1
        if (o3 == 0.0 && onSegment(p2, p1, q2)) return true // p2, q2 and p1 are collinear and p1 lies on p2q2
        return o4 == 0.0 && onSegment(
            p2,
            q1,
            q2
    ...
    private fun sign(num: Double): Double {
        return if (num > 0) 1.0 else if (num < 0) -1.0 else 0.0
    }
    ...
    private fun zOrder(x: Double, y: Double, minX: Double, minY: Double, invSize: Double): Double {
        // coords are transformed into non-negative 15-bit integer range
        var lx: Int = (32767 * (x - minX) * invSize).toInt()//TODO ver oq fazia aqui o método Double.valueOf
        var ly: Int = (32767 * (y - minY) * invSize).toInt()//TODO ver oq fazia aqui o método Double.valueOf
    ...
    private fun indexCurve(start: Node?, minX: Double, minY: Double, invSize: Double) {
        var p: Node? = start
        do {
            if (p!!.z == Double.MIN_VALUE) p.z = zOrder(p.x, p.y, minX, minY, invSize)
            p.prevZ = p.prev
            p.nextZ = p.next
            p = p.next
        } while (p !== start)
    ...
    private fun eliminateHoles(data: DoubleArray, holeIndices: IntArray?, outerNode: Node, dim: Int): Node {
        ...
        val queue: MutableList<Node?> = mutableListOf<Node?>()
        ...
        queue.sortWith(
            object : Comparator<Node?> {
                override fun compare(o1: Node?, o2: Node?): Int {
                    if (o1!!.x - o2!!.x > 0) return 1 else if (o1.x - o2.x < 0) return -2//TODO tive que forçar o !! aqui
                    return 0
                }
            }
        )
    ...
    private class Node internal constructor(var i: Int, var x: Double, var y: Double) {
        var z: Double
        var steiner: Boolean
        var prev: Node? = null
        var next: Node? = null
