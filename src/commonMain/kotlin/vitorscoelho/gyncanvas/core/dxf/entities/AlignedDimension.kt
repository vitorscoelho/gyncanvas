package vitorscoelho.gyncanvas.core.dxf.entities

import vitorscoelho.gyncanvas.core.dxf.Color
import vitorscoelho.gyncanvas.core.dxf.DimStyleOverrides
import vitorscoelho.gyncanvas.core.dxf.tables.DimStyle
import vitorscoelho.gyncanvas.core.dxf.tables.Layer
import vitorscoelho.gyncanvas.math.Vector
import kotlin.math.atan
import vitorscoelho.gyncanvas.math.toRadians

data class AlignedDimension(
    override val layer: Layer,
    override val color: Color = Color.BY_LAYER,
    override val dimStyle: DimStyle,
    override val dimStyleOverrides: DimStyleOverrides = DimStyleOverrides.NONE,
    override val xPoint1: Vector,
    override val xPoint2: Vector,
    private val dimensionLineReferencePoint: Vector,
    override val text: String = "<>"
) : LinearDimension by RotatedDimension(
    layer = layer,
    color = color,
    dimStyle = dimStyle,
    dimStyleOverrides = dimStyleOverrides,
    xPoint1 = xPoint1,
    xPoint2 = xPoint2,
    dimensionLineReferencePoint = dimensionLineReferencePoint,
    text = text,
    angle = run {
        val points = arrayOf(xPoint1, xPoint2)
        val rightPoint: Vector = points.maxByOrNull({ it.x })!!
        val leftPoint: Vector = points.minByOrNull({ it.x })!!
        val delta = rightPoint - leftPoint
        if (delta.x == 0.0) return@run toRadians(90.0)
        return@run atan(delta.y / delta.x)
    }
) {
    override fun dimContinue(point: Vector): AlignedDimension = dimContinueXPoint2(point)

    override fun dimContinueXPoint1(point: Vector): AlignedDimension {
        return copy(xPoint1 = this.xPoint1, xPoint2 = point)
    }

    override fun dimContinueXPoint2(point: Vector): AlignedDimension {
        return copy(xPoint1 = this.xPoint2, xPoint2 = point)
    }

    override fun createSequence(points: List<Vector>): List<AlignedDimension> {
        val dimensions = mutableListOf(this)
        points.forEach { dimensions.add(dimensions.last().dimContinue(it)) }
        return dimensions
    }
}