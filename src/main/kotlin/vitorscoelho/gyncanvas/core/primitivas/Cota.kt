...
class CotaHorizontal(
    override val ponto1: Vector2D,
    override val ponto2: Vector2D,
    ...
    private val dimensionLine = StrokedLine(
        ponto1 = Vector2D(x = ponto1.x, y = yDimensionLine),
        ponto2 = Vector2D(x = ponto2.x, y = yDimensionLine)
    )
    ...
    companion object {
        ...
        private fun criarExtensionLine(
            ...
            return StrokedLine(
                ponto1 = Vector2D(x = definitionPoint.x, y = yDefinition),
                ponto2 = Vector2D(x = definitionPoint.x, y = yDimensionLine)
            )
        }
    }
}
...
class CotaVertical(
    ...
    private val dimensionLine = StrokedLine(
        ponto1 = Vector2D(x = xDimensionLine, y = ponto1.y),
        ponto2 = Vector2D(x = xDimensionLine, y = ponto2.y)
    )
    private val extensionLine1 = criarExtensionLine(
        definitionPoint = ponto1,
    ...
    companion object {
        ...
        private fun criarExtensionLine(
            ...
            return StrokedLine(
                ponto1 = Vector2D(x = xDefinition, y = definitionPoint.y),
                ponto2 = Vector2D(x = xDimensionLine, y = definitionPoint.y)
            )
        }
    }
}
...
private class CotaInclinada(
    ...
    init {
        ...
        val distancia = ponto1.distance(ponto2)
        //Considerando que a cota é horizontal, para depois rotacionar e transladar estes pontos para formar a cota
        val cotaHorizontalPonto1 = Vector2D.ZERO
        val cotaHorizontalPonto2 = cotaHorizontalPonto1.plus(deltaX = distancia)
        val pontoTextoCotaHorizontal = Vector2D(x = distancia / 2.0, y = offset + propriedadesCotas.offsetText)
        val definitionPoint1CotaHorizontal = cotaHorizontalPonto1.plus(deltaY = offset)
        val definitionPoint2CotaHorizontal = cotaHorizontalPonto2.plus(deltaY = offset)
        ...
        val inicioLinhaDeCotaHorizontalPonto1 = cotaHorizontalPonto1.plus(deltaY = yInicialLinhaDeCota)
        val inicioLinhaDeCotaHorizontalPonto2 = cotaHorizontalPonto2.plus(deltaY = yInicialLinhaDeCota)
...
abstract class SequenciaCota<T : Cota>(
    ...
    fun add(ponto: Vector2D, texto: String = "<>"): SequenciaCota<T> {
        pontos += ponto
        textos += texto
        return this
    }
    ...
    fun addDelta(deltaX: Double = 0.0, deltaY: Double = 0.0, texto: String = "<>"): SequenciaCota<T> {
        return add(ponto = pontos.last().plus(deltaX = deltaX, deltaY = deltaY), texto = texto)
    }

    fun add(x: Double, y: Double, texto: String = "<>"): SequenciaCota<T> {
        return add(ponto = Vector2D(x = x, y = y), texto = texto)
    }
    ...
    protected abstract fun criarCota(ponto1: Vector2D, ponto2: Vector2D, texto: String): T
}
...
class SequenciaCotaHorizontal(
    ...
    override fun criarCota(ponto1: Vector2D, ponto2: Vector2D, texto: String) =
        CotaHorizontal(
            ponto1 = ponto1,
            ponto2 = ponto2,
            yDimensionLine = yDimensionLine,
